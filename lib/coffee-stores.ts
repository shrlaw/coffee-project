...
const transformCoffeeData = (
  ...
  console.log('Raw result passed to transformCoffeeData:', result);

  // assign to a variable so we can log it
  const coffeeStore = {
    id: result.id,
  ...
  console.log('Transformed coffeeStore:', coffeeStore); // check that address is correct
  return coffeeStore; // still return it
};
